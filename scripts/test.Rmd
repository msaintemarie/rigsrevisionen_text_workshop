...
1. Import the 'doc_id' and 'token' columns from the corpus using the *data.table::fread()* function. Save this data table under the name of your choice using the *data.table:: function. fwrite()*. This file will be reused for a later exercise.
...
head(corpus[
  ministerområde == 'Udenrigsministeriet'])
...
corpus_dates <- unique(corpus[,.(doc_id, fremsat_dato)])
...
corpus_dates <-
  corpus_dates[, c('year',
                   'month',
                   'day',
                   'fremsat_dato') := .(data.table::year(fremsat_dato),
                                        data.table::month(fremsat_dato),
                                        data.table::mday(fremsat_dato),
                                        NULL)]
```
...
data.table::fwrite(corpus_date, here::here('data/processed',
                                           'corpus_date.txt'))
...
#                  by = .(source = Publication )] %>%
#             .[order(tkoens, decreasing = TRUE)], n = 5))

# How do you interpret this command?
```
...
raw_corpus <-
  corpus[, .(document_text = gsub('( )([[:punct:]])', '\\2',
...
raw_corpus <-
  corpus[,.(document_text = mgsub::mgsub(
    paste(token, collapse = " "),
    c('( )([[:punct:]])', "\\' ",
      ' à le ', ' à les ',
#                           ' À le ', ' À les ',
...
raw_corpus[texte_article %like% 'mink']
...
corpus[token == 'mink', .(doc_id, paragraph_id, sentence_id)] %>%
  merge(., corpus, by = c('doc_id', 'paragraph_id', 'sentence_id')) %>%
  .[, .(word_context = paste(token, collapse = " ")), by = doc_id] %>%
  .[, word_context]
...
corpus[, .(tokens = .N),
       by = .(Ministerområde)] %>%
  .[order(tokens, decreasing = TRUE)] %>%
  ggplot2::ggplot(aes(x = tokens, y = ministerområde)) +
  ggplot2::geom_col()
...
corpus[, .(tokens = .N, types = uniqueN(token)),
  ...
  .[order(tokens, decreasing = TRUE)] %>%
  data.table::melt(.,
                   id.vars = 'ministerområde',
                   variable.name = 'lexical_type',
